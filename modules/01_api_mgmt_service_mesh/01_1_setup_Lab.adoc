...
+
Follow the directions exactly as indicated to ssh into your remote lab VM.
...
$ echo "export API_PASSWD=<provided by your instructor>" >> ~/.bashrc
...
. On your client VM, access to the `root` operating system can be achieved by executing: `sudo -i`
. As the `root` operating system user, `cluster admin` to your OCP environment can be achieved by executing :
+
-----
