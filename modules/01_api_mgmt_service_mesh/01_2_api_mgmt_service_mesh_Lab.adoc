...
spec:
  ...
  - port:
    ...
    hosts:
    - \"*\"" \
 > $HOME/lab/catalog-mixer-gw.yml
-----

. Create gateway:
+
-----
$ oc create -f ~/lab/catalog-mixer-gw.yml -n istio-system --as=system:admin
-----
...
spec:
  ...
  - match:
    ...
    route:
    - destination:
        host: "threescaleistioadapter"
        port:
          number: 3333" \
> $HOME/lab/catalog-mixer-vs.yml
-----

. Create Gateway & Virtual Service:
+
-----
$ oc create -f ~/lab/catalog-mixer-vs.yml -n istio-system --as=system:admin
-----
