...
It lowers the {transaction costs} (ie: search costs, price discovery, policing and enforcement costs, etc ) that would otherwise hinder mutually beneficial exchange between API consumer and producer.
...
You do this by applying both a _service mesh_ and an _API Management_ solution to an existing _cloud native_ application running in Red Hat's Openshift Container Platform.


The lab takes you through a journey that has you gradually apply more and more features of both _API Mgmt_ and _service mesh_ to your cloud native application.

=== Background

Red Hat has and continues to make extensive investments in technologies that foster Cloud Native Applications.

. *Dev / Ops*
+
With its _Openshift_ series of offerings, Red Hat is a significant innovator in the technologies used to facilitate _Dev Ops_.

. *API Management*
+
_Gartner's Magic Quadrant for Full Life Cylce API Management_ puts Red Hat's 3scale API management solution in the leader's quadrant.
...
Specific to API Management and MSA _Service Mesh_, the following table provides a broad feature comparison :
...
This lab takes you through a journey that has you gradually apply more and more features of both _API Mgmt_ and _service mesh_ to your cloud native application.

As the first step in this journey, this section of the lab has you provision a supported version of 3scale _API Gateway_ to manage your CoolStore _catalog_ service.
...
Once complete, continue on with the next section: <<apicast_istio>>
...
[blue]#Congradulations!#Your lab environment should now consist of a _catalog_ RESTful service managed by out of the box Red Hat 3scale API Manager 2.2 functionality.

#[blue]#This is a substantial accomplishment!# 
You are now utilizing the tools needed to lower the {transaction costs} (ie: search costs, price discovery, policing and enforcement costs, etc ) that would otherwise hinder mutually beneficial exchange between API consumer and producer.
Via the Red Hat 3scale API Management product, you can manage the entire life-cycle of your APIs and provide value-add to your customers and partners.
...
Admittedly, simple injection of the _Envoy_ sidecar into your API gateway is likely of limited value. 
However, this remains early in our journey.
You will continue to build upon the foundations set here in later sections of this lab.
...
-----
+
You'll use the definition of your existing API gateway as a template to create a new _istio_ enabled API gateway.
...
Please note that the cluster admin of the service mesh could have optionally automated the injection of these Envoy configs into all _deployments_ by default.
...
If so, you'll need to troubleshoot the root cause of why either the _liveness_ and/or _readiness_ probes are failing.
...
You've essentially conducted a simple _A / B Deployment_ at the _service_ layer where 100% of all traffic through the existing unmodified route will flow to this new Istio enabled pod.
...
[blue]#As mentioned previously, the value of what you've accomplished thus far from a non-functional requirements perspective is admittedly of minimal value.
However, you are still early in this journey toward the full utilization of API Management and service mesh of your Container Native application.
You'll continue to build upon what you've accomplished thus far in the next sections of this lab#

== Istio Ingress gateway

Up until now, incoming traffic into the production API gateway has been directly via the out-of-the-box _HAProxy_ based Kubernetes / OCP _router_.

This has worked fine however what is missing is the ability to apply more sophisticated route rules as well as integration with service mesh _observability_ tools such as _Jaeger_ and _Prometheus_.
Istio includes an _ingress_ gateway that implements these additional features.

In this section of the lab, you will modify the flow of inbound traffic such that it additionally flows through the _Ingress_ gateway of Istio.
Among other benefits, you'll be able to trace inbound traffic as it initially enters your solution.
...
. The previous smoke test utilized an $INGREGRESS_HOST and $INGRESS_PORT that are made available on the local OpenShift sub-net.
...
You'll do so in this section by utilizing an _Opentracing_ enabled variant of the 3scale API Gateway.
...
+
-----
$  oc get service jaeger-agent -n istio-system --as=system:admin
...
.. In the drop down for `Operation`, there are likely only two options available: `all` and `apicast_management`.
+
At this time, neither of these operations are particularly relevant to our use case.
The `apicast_management` operation pertains to the invocations made by the _readiness_ and _liveness_ probes of the deployment to the API gateway pod.
...
.. Return back the Jaeger UI and notice more options present in the drop down for `Operation`.
+
...
-----
$ oc patch deploy/$OCP_USERNAME-cat-service-istio \
   --patch '{"spec":{"template":{"spec":{"containers":[{"name":"'$OCP_USERNAME'-cat-service-istio", "image": "docker.io/rhtgptetraining/catalog-service-tracing:1.0.15" }]}}}}' \
   -n $MSA_PROJECT
-----
...
. From the terminal, use the curl utility as you've done so in the past to invoke your catalog service several times via the Istio ingress :
...
Subsequently, the API gateways along with any associated routing rules can be deleted.
...
Let's create a routing rule to invoke the backend catalog service directly via the Istio _ingress_ gateway.

Later, more complex routing rules will modify the flow to re-direct through the 3scale Istio Mixer adapter.
...
.. With the 1.0.0 version of community Istio,  sometimes the Istio policy can get itself in an unstable state.
Before progressing with management of more sophisticated routing rules to support the 3scale Istio Mixer, its a good idea to bounce the _policy_ pod.
...
2018-09-17T23:48:27.516971Z     warn    Neither --kubeconfig nor --master was specified.  Using the inClusterConfig.  This might not work.
2018-09-17T23:48:27.517481Z     info    Built new config.Snapshot: id='0'
2018-09-17T23:48:27.517576Z     info    Cleaning up handler table, with config ID:-1
2018-09-17T23:48:57.664804Z     info    Built new config.Snapshot: id='1'
2018-09-17T23:48:57.664913Z     info    adapters        getting kubeconfig from: ""     {"adapter": "handler.kubernetesenv.istio-system"}
2018-09-17T23:48:57.664936Z     warn    Neither --kubeconfig nor --master was specified.  Using the inClusterConfig.  This might not work.
2018-09-17T23:48:57.665937Z     info    adapters        Waiting for kubernetes cache sync...    {"adapter": "handler.kubernetesenv.istio-system"}
2018-09-17T23:48:57.766175Z     info    adapters        Cache sync successful.  {"adapter": "handler.kubernetesenv.istio-system"}
2018-09-17T23:48:57.770506Z     info    Cleaning up handler table, with config ID:0
2018-09-17T23:48:57.776693Z     info    adapters        serving prometheus metrics on 42422     {"adapter": "handler.prometheus.istio-system"}
Istio Mixer: root@71a9470ea93c-docker.io/istio-1.0.0-3a136c90ec5e308f236e0d7ebb5c4c5e405217f4-Clean
Starting gRPC server on port 9091
2018-09-17T23:48:57.864365Z     info    ControlZ available at 172.17.0.10:9876
-----
...
It's not needed in these smoke tests because this inbound traffic is no longer flowing through your API gateway.
...
.. View details of that recently posted catalog item:
+
-----
...
.. For the purpose of this lab, the source code utilized is an unmodified fork of the upstream community at:  `https://github.com/3scale/istio-integration`
.. Execute:
+
-----
$ git clone \
...
. Set the 2 containers at the debug log level:
+
-----
...
-----
+
Embedded in these yaml files is the 3scale handler that gets injected into the Istio Mixer.
The (less then ideal) alternative to injecting a mixer adapter would have been to develop the 3scale handler directly in the Istio mixer source code and then compile and deploy this entire modified Istio mixer.
...
$ oc patch rule/usethreescale \
       --type=json \
       --patch '[{"op": "add", "path": "/spec/match", "value":"destination.service == \"catalog-service.'$MSA_PROJECT'.svc.cluster.local\" && source.namespace != \"'$MSA_PROJECT'\" && match(request.path,\"/product\")"  }]' \
       -n istio-system \
       --as=system:admin
-----
+
This modification introduces a fine grained _match_ with the following conditions such that only when all of them are met will traffic be routed to the 3scale mixer adapter:
