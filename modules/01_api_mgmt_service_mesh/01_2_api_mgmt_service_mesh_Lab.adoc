...
This is worked fine however what is missing is the ability to apply more sophisticated route rules as well as integration with service mesh _observability_ tools such as Jaeger and Promtheus.
Istio includes an _ingress_ gateway that implements this additional features.
...
It should match what's been configured in the _catalog-service_ in the API Manager.
+
image::images/recall_prod_base.png[]


. Create a file that defines a ingress gateway:
+
-----
echo \
    "apiVersion: networking.istio.io/v1alpha3
...
. In the details of your _catalog_ service the API Manager admin console, locate the `ID for API calls ... `: 
+
