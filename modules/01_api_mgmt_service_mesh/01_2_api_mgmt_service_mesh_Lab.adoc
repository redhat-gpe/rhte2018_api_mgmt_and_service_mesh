...
. In your course virtual machine, ensure you are the same non-root user in which you previously set lab related environment variables.
...
In a real-world setting, you would have coordinated with a team-member who does have _cluster admin_ rights to execute this command for you.
...
Please refer to the <<istio_troubleshooting>> in the Appendix of this lab for more tips.
...
. Set environment variables that reflect the host and port of the Istio Ingress Gateway resource:
+
-----
...
+
-----
$ curl -v -k  -HHost:$CATALOG_API_GW_HOST `echo "http://"$(oc get route istio-ingressgateway -n istio-system -o template --template {{.spec.host}})"/products?user_key=$CATALOG_USER_KEY"`
-----
...
WARNING:  This section pertains to a stretch goal for this lab and remains a work in progress.
If by 10 Sept, i can't get it to go .... I will comment this entire section out.
...
. Ensure the $CATALOG_API_GW_HOST environment variable still remains in your shell:
+
-----
$ echo $CATALOG_API_GW_HOST
...
spec:
  ...
  - port:
    ...
    hosts:
    - "$CATALOG_API_GW_HOST"" \
 > $HOME/lab/catalog-direct-gateway.yml
-----

. Create gateway:
+
-----
$ oc create -f ~/lab/catalog-direct-gateway.yml -n $MSA_PROJECT --as=system:admin
-----
...
spec:
  ...
  - match:
    ...
    - destination:
        ...
        port:
          number: 8080" \
> $HOME/lab/catalog-direct-vs.yml
-----

. Create Virtual Service:
+
-----
$ oc create -f ~/lab/catalog-direct-vs.yml -n $MSA_PROJECT --as=system:admin
-----

=== 3scale Istio Mixer
. Begin by cloning the source code to the 3scale Istio Mixer :
+
-----
$ git clone \
...
. Load the Istio Mixer Adapter into the istio-system namesapce:
+
-----
...
+
-----
$ oc get handler -n istio-system --as=system:admin

....

threescalehandler   1d
-----
