...
[blue]#The lab takes you through a journey that has you gradually apply more and more features of both _API Mgmt_ and _service mesh_ to your cloud native application.#
...
As the first step in this journey, you smoke test the management of your _catalog_ RESTful services using your API Manager and API gateways.

A deployment topology diagram of the components involved in this API managed environment are as follows:
...
.. *MAPPING RULES*
+
Add an additional `Mapping Rule` for the HTTP POST method:
+
You'll use the POST method in the last section in this lab.

... In the `MAPPING RULES` section, click the green link: `Add Mapping Rule`
... In the new mapping rule, change the verb to `POST` and set `/` as the Pattern
+
...
[blue]#Congratulations!# Your lab environment should now consist of a _catalog_ RESTful service managed by out of the box Red Hat 3scale API Manager 2.2 functionality.

[blue]#This is a substantial accomplishment!#
You are now utilizing the tools needed to lower the {transaction_costs} (ie: search costs, price discovery, policing and enforcement costs, etc ) that would otherwise hinder mutually beneficial exchange between API consumer and producer.
...
-----
+
The _jaeger-agent_ receives tracing information (in the form of UDP packets on port 6831) submitted by jaeger client libraries embedded in apps and forwards in batch to the Jaeger collector.
+
...
. If you are not already there, return back to the terminal window of your lab, and use the `curl` utility to invoke your catalog service (via:  istio ingress gateway -> API gateway) a few times.
+
-----
$ curl -v \
       -HHost:$CATALOG_API_GW_HOST \
...
The _catalog service_ is link:https://github.com/gpe-mw-training/catalog-service[written in Java] (specifically using the _reactive_ programming framework: link:https://vertx.io/[vert.x]).
...
The source code of the OpenTracing enabled catalog service can be found link:https://github.com/gpe-mw-training/catalog-service/tree/jaeger-rht[here].
...
. The original catalog service is no longer needed.  Go ahead and scale it down as follows:
+
-----
