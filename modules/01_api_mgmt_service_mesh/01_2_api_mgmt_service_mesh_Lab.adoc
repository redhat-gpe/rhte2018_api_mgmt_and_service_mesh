...
.. Add an additional `Mapping Rules` for the HTTP POST method:
+
...
.. For the purpose of this lab, allow deployments in the _istio-system_ namespace to deploy without specifying limits and requests.
+
Do so by deleting the clusterquota that is currently associated with the user:  $OCP_USERNAME:
+
-----
$ oc delete clusterquota clusterquota-rhte-mw-api-mesh-$OCP_USERNAME --as=system:admin
-----
+
NOTE: This step will be made un-necessary in a future version of this lab.
All OCP namespaces (to include the istio-system namespace) will be enabled with a _LimitRange_ that defines default limits and requests.
...
[blue]#Congradulations! The catalog service is backed to being managed and secured by the 3scale API Manager.#
