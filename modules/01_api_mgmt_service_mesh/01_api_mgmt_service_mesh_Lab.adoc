...
$ export OCP_PROJECT=rhte-mw-api-mesh-$OCP_USERNAME
-----
...
+
-----
$ oc login https://master.$REGION.openshift.opentlc.com -u $OCP_USERNAME -p <OCP user password provided by instructor>
-----
...
The connection problem will be in the apicast gateway at boot when it attempts to pull (using the value set in its THREESCALE_PORTAL_ENDPOINT env variable) _proxy-config_ information from the _backend-listener_ of 3scale AMP.
...
kind: ServiceEntry
metadata:
  name: customer-apicast-egress-rule-$OCP_USERNAME
spec:
  hosts:
  ...
  - name: https-443
    ...
    protocol: HTTPS 
  resolution: DNS" \
 > $HOME/lab/customer-apicast-egressrule.yaml
-----
...
oc logs -f  `oc get pod | grep "apicast-istio" | awk '{print $1}'` -c catalog-prod-apicast-istio        :   log of istio enabled apicast gw

for i in `oc get pod | grep "apicast-istio" | awk '{print $1}'`; do oc delete pod $i; done              :   Re-dploy Istio enabled Apicast gateway


TO-DO:
  1) investigate istio-ingress
        OCP ha-proxy -> istio-ingress -> apicast gw -> catalog service

  2) when apicast is in info log level, why does it stop rebooting itself when a THREESCALE_PORTAL_ENDPOINT related problem is encountered ?
     when apicast is in debug log level, it continues to cycle when it encounters a THREESCALE_PORTAL_ENDPOINT problem .

  3) with istio injected apicast, boot doesn't start however a curl within the same pod on THREESCALE_PORTAL_ENDPOINT does work

  4) allow user write access to istio-system to allow for execution of:  "istioctl create"
