...
==== OpenTracing vs Istio
 
Of related interest is that the upstream community APIcast gateway from Red Hat's 3scale product already bundles _OpenTracing_ client library using _Jaeger_.
This already allows it to participate in distributed tracing where the traces and spans could be viewed in a standalone Jaeger installation as discussed in link:https://medium.com/@jmprusi_49013/adding-opentracing-support-to-apicast-api-gateway-a8e0a38347d2[this tutorial].

As part of its _control plane_, Istio bundles in Jaeger. 
...
This apicast gateway is similar to the supported APIcast gateway that is included with 3scale AMP.
Its differences with the supported APIcast are as follows:

. Management of this APIcast gateway occurs via a Kubernetes _deployment_ as opposed to an OCP _deploymentconfig_. 
+
The reason for this change is discussed previously in the section: <<dvsdc>>.
. The APIcast gateway used in this lab includes an additional library called:  _libjaegertracing.so_ .
This library provides support for the _OpenTracing_ specification using _Jaeger_.

As in the supported variant, your APIcast gateway will pull _proxy_ configuration data from the pre-provisioned multi-tenant 3scale AMP using the value of: $THREESCALE_PORTAL_ENDPOINT.
...
+
-----
$ oc rollover resume deploy catalog-stage-apicast catalog-prod-apicast -n $OCP_PROJECT
-----
...
In the above diagram, notice the introduction of a new pod: _catalog-prod-apicast-istio_.
Ingress requests through the route and service are now directed to this new Istio enabled APIcast pod (instead of the original apicast that is not Istio enabled).

The _Envoy_ proxy of your _catalog-prod-apicast-istio_ pod will interoperate with _service mesh control plane_ functionality found in the _istio-system_ namespace.
...
-----
+
TO-DO:  provide sufficient credentials to users to execute this command.
...
Often the first thing to understand about your microservices architecture is specifically which microservices are involved in and end-user transaction.
Istio's Mixer comes with the ability to pull tracing spans from your distributed microservices.

Although Istio supports both Zipkin and Jaeger, for our purposes we focus on Jaeger, which implements OpenTracing, a vendor neutral tracing API.
Jaeger was originally open sourced by the Uber Technologies team and is a distributed tracing system specifically focused on microservices architecture.
...
oc logs -f  `oc get pod | grep "apicast-istio" \
            | grep "Running" \
            | awk '{print $1}'` -c $OCP_USERNAME-catalog-prod-apicast-istio                                         :   log of istio enabled apicast gw
...
-----
$ oc create configmap jaeger-config --from-file=$HOME/lab/jaeger_config.json
-----

-----
$ oc volume deploy/$OCP_USERNAME-catalog-prod-apicast-istio --add -m /tmp/jaeger/ --configmap-name jaeger-config
-----

-----
$ oc env deploy/$OCP_USERNAME-catalog-prod-apicast-istio OPENTRACING_TRACER=jaeger OPENTRACING_CONFIG=/tmp/jaeger/jaeger_config.yaml
-----

Failed to open tracer configuration file /tmp/jaeger/jaeger_config.jso (2: No such file or directory)

Failed to construct tracer: `service_name` not provided
