...
Your lab assets should now consist of a _catalog_ RESTful service managed by out of the box 3scale AMP 2.2 functionality.

In this section of the lab, you will now use a new APIcast gateway that includes _OpenTracing_ libraries and is injected with the Istio _Envoy_ proxy.

image::images/deployment_apicast-istio.png[]

In the above diagram, notice the introduction of a new pod: _prod-apicast-istio_.
Ingress requests through the route and service are now directed to this new OpenTracing / Istio enabled APIcast pod (instead of the original 3scale AMP 2.2 apicast that is not OpenTracing enabled).

. More specifically, the APIcast gateway used in this section of the lab includes an additional library called:  _libjaegertracing.so_ .
...
. Switch to use of Opentracing and Jaeger version of APIcast
+
-----
...
{
    "service_name": "$OCP_USERNAME-apicast",
    "disabled": false,
    "sampler": {
      "type": "const",
      "param": 1
...
-----
$ oc env deploy/$OCP_USERNAME-prod-apicast-istio OPENTRACING_TRACER=jaeger OPENTRACING_CONFIG=/tmp/jaeger/jaeger_config.json -n $OCP_PROJECT
-----
...
- name: OPENTRACING_TRACER
          ...
          - name: OPENTRACING_CONFIG
            value: "/tmp/jaeger/jaeger_config.json"


== Jaeger UI

Thus far, you have Istio enabled your APIcast gateway pod.
Along with all other components that will eventually run in your Istio service mesh, it is subject to its various policies .

This next section of the lab introduces you to _OpenTracing_ and _Jaeger_.

OpenTracing is a consistent, expressive, vendor-neutral APIs for distributed tracing and context propagation.

Jaeger is one of several implementations of OpenTracing.

Istio provides Jaeger functionality out of the box.
