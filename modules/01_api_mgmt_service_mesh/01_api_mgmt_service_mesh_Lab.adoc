...
Ingress requests through the route and service are now directed to this new Istio enabled APIcast pod.
...
You'll make use of these opentracing client libraries in a later section of this lab.

. You'll be making quite a few changes to your Istio enabled apicast gateway.  Put it in a paused state while those changes are being made:
+
-----
...
. Create a json config file that instructs the _nginx-opentracing_ and _jaeger-cpp_ client libraries in the apicast gateway how to push traces to the Jaeger agent:
+
-----
...
+
The opentracing libraries in your apicast will push traces in the form of UDP packets to this jaeger-agent host and port.

.. You can verify that the jaeger-agent actually exists by executing and is expecting UDP packets on port 6831:
+
-----
...
. Create a configmap from opentracing json file:
+
-----
$ oc create configmap jaeger-config --from-file=$HOME/lab/jaeger_config.json -n $OCP_PROJECT
-----

. Mount the configmap to your Istio and opentracing  enabled apicast:
+
-----
...
. Switch to the use an APIcast gateway that is Opentracing and Jaeger enabled:
+
-----
$ oc patch deploy/$OCP_USERNAME-prod-apicast-istio \
...
. Verify opentracing related libraries in the APIcast gateway.
+
Once your apicast is back up and running, execute commands such as the following :
+
-----
...
This next section of the lab introduces you to _Jaeger_ user inferface.
