...
An _Istio_ based service mesh introduces features that somewhat resemble features found in an API management solution.
...
This allows it already to participate in distributed tracing when paired with implementations such as Zipkin or Jaeger.
...
TO-DO :  Vacuum will elaborate on this on Aug 9 during the 3scale F2F .
...
Details about these Red Hat development platforms are out of scope for this lab.
...
This apicast gateway is similar to the supported APIcast gateway that is included with 3scale AMP with the exception that management of this APIcast gateway occurs via a Kubernetes _deployment_ as opposed to an OCP _deploymentconfig_. 
