...
$ oc new-app \
     ...
     -n $LAB_PROJECT > /tmp/catalog-stage-apicast_details.txt
-----
...
$ oc new-app \
     ...
     -n $LAB_PROJECT > /tmp/catalog-prod-apicast_details.txt
-----

. Resume the intially paused deploy objects:
+
-----
$ oc rollout resume deploy catalog-stage-apicast catalog-prod-apicast
-----
...
+
-----
$ echo -en "\n`oc get route catalog-stage-apicast-$OCP_USER_ID -n $LAB_PROJECT --template "https://{{.spec.host}}"`:443\n\n"
-----

.. *Production Public Base URL*: Populate this field with the output from the following command:
+
-----
$ echo -en "\n`oc get route catalog-prod-apicast-$OCP_USER_ID -n $LAB_PROJECT --template "https://{{.spec.host}}"`:443\n\n"
-----
...
Every time a configuration change is made to a api proxy or application plan, the production APIcast gateways need to be refreshed with the latest changes.
