...
+
-----
$ echo -en "\nhttps://master-$OCP_REGION.generic.opentlc.com\n\n"
-----
...
+
-----
# wget https://bit.ly/2G2GQWr -O $HOME/lab/api_manager_state_update.sh \
...
.. You can tail the pod containig the ansible responsible for deletion by executing the following:
+
-----
$ oc logs -f `oc get pod -n istio-system | grep "istio-removal" | awk '{print $1}'` -n istio-system
-----

.. Also, `oc get project -w` will indicate when this namespace deletion occurs
