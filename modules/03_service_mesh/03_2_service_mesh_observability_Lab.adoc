...
$ oc set env deploy/$OCP_USERNAME-prod-apicast-istio \
         ...
         OPENTRACING_HEADER_FORWARD=uber-trace-id \
         -n $GW_PROJECT
-----
+
TO_DO:   Should uber-trace-id or x-request-id (from https://istio.io/docs/tasks/telemetry/distributed-tracing/overview/) be used ??

. Inject a _jaeger-agent_ sidecar:
+
-----
-----
