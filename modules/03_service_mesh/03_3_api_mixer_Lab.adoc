...
.. For the purpose of this lab, the source code utilized is a fork of the upstream community at link:https://github.com/3scale/istio-integration[].
.. Execute:
+
-----
$ git clone \
...
+
As per the diagram above, the container provides the following two components:
...
+
-----
$ oc create -f $HOME/lab/istio-integration/istio/threescale-adapter-config.yaml --as=system:admin
...
+
-----
$ oc get handler threescale -n istio-system --as=system:admin -o yaml
